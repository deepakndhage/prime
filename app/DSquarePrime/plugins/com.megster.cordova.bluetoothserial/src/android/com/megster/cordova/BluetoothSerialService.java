...
public class BluetoothSerialService {
    ...
    private synchronized void setState(int state) {
        if (D) Log.d(TAG, "dsquare-BT -" +Thread.currentThread().getId() + " - setState() " + mState + " -> " + state);
        mState = state;
	...
	if (D) Log.d(TAG, "dsquare-BT -" +Thread.currentThread().getId() + " -  changing state to 0 "+new Date());
        setState(STATE_NONE);
...
//        }
    ...
    public synchronized void connect(BluetoothDevice device, boolean secure) {
        if (D) Log.d(TAG, "dsquare-BT -" +Thread.currentThread().getId() + " - connect to: " + device);
	...
	if (D) Log.d(TAG, "dsquare-BT -" +Thread.currentThread().getId() + " - connected - " + socketType +" -- "+new Date());
	...
	if (D) Log.d(TAG, "dsquare-BT -" +Thread.currentThread().getId() + " -  giving up "+new Date());
        Message msg = mHandler.obtainMessage(BluetoothSerial.MESSAGE_TOAST);
        Bundle bundle = new Bundle();
        bundle.putString(BluetoothSerial.TOAST, log+"-- Unable to connect to device");//"Unable to connect to device"
        msg.setData(bundle);
        mHandler.sendMessage(msg);
	if (D) Log.d(TAG, "dsquare-BT -" +Thread.currentThread().getId() + " -  UI should see failure "+new Date());
	...
	//private String log="";
	    ...
	    //log=log+"-- Connecting to socket..."+mConnectThread.getId();
            mmSocket = tmp;
        }
	...
	public synchronized void connectItToBT(){
		log = "";
            Log.i(TAG, "BEGIN mConnectThread SocketType:" + mSocketType);
	    log=log+"-- BEGIN mConnectThread SocketType:" + mSocketType;
		...
		log=log+"-- Connecting to socket..."+mmSocket;
		if (D) Log.d(TAG, "dsquare-BT -" +Thread.currentThread().getId() + " -  trying to connect "+new Date());
                mmSocket.connect();
                Log.i(TAG,"Connected");
		log=log+"-- Connected";
            } catch (IOException e) {
		...
		log=log+e.toString();
			...
			log=log+"-- Trying fallback...";
			if (D) Log.d(TAG, "dsquare-BT -" +Thread.currentThread().getId() + " - trying fallback "+new Date());
			StringWriter sw = new StringWriter();
			PrintWriter pw = new PrintWriter(sw);
			e.printStackTrace(pw);
			String trace = sw.toString();
			log=log+" -- Exc T_ID1 - "+ Thread.currentThread().getId() +" TRACE " +trace;
			...
			log=log+"Couldn't establish a Bluetooth connection - "+e2.toString();
			...
			//log=log+" -- Exc - "+fileName+" * "+methodName+" * "+line+" * "+col;
			//String trace = org.apache.commons.lang.exception.ExceptionUtils.getStackTrace(e2);
			//log=log+" -- Exc - "+trace;
			...
			String trace = sw.toString();
			log=log+" -- Exc T_ID2 - "+ Thread.currentThread().getId() +" TRACE " +trace; 
			if (D) Log.d(TAG, "dsquare-BT -" +Thread.currentThread().getId() + " -  failed in fallback "+new Date());
                    try {
                        mmSocket.close();
                    } catch (IOException e3) {
                        Log.e(TAG, "unable to close() " + mSocketType + " socket during connection failure", e3);
			if (D) Log.d(TAG, "dsquare-BT -" +Thread.currentThread().getId() + " -  failed to close socket "+new Date());
                    }
                    connectionFailed();
                    return;
                }
	    ...
	    if (D) Log.d(TAG, "dsquare-BT -" +Thread.currentThread().getId() + " - connected "+new Date());
