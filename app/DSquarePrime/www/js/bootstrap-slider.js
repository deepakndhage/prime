	...
	Slider.prototype = {
		...
		layout: function(){
			...
			if (this.range) {
				...
				$('#'+this.dimmerPerc).html(this.formater(this.value[0]));
			}
		},
		...
		mousemove: function(ev) {
			...
			var bulbId=$('#'+this.element.attr('id')+'-id').html();
			var dataToSend = [123, bulbId, val, val-bulbId];
		...
		mouseup: function(ev) {
			...
			if(val == 0){
				if($('#'+this.element.attr('id')+'-bulb').attr('src') && $('#'+this.element.attr('id')+'-bulb').attr('src')=="./images/bulb-on.png"){
	        	 $('#'+this.element.attr('id')+'-bulb').attr('src','./images/bulb-off.png'); 
	        	}
	        	 ControllerPlugin.saveState('update', $('#'+this.element.attr('id')+'-id').html(), 0,0,7);
	        	// ControllerPlugin.showToast('showMessage', '7');
	      }
	      else if(val > 0){
				if($('#'+this.element.attr('id')+'-bulb').attr('src') && $('#'+this.element.attr('id')+'-bulb').attr('src')=="./images/bulb-off.png"){
	        	  $('#'+this.element.attr('id')+'-bulb').attr('src','./images/bulb-on.png');
	        	}
	        	 ControllerPlugin.saveState('update', $('#'+this.element.attr('id')+'-id').html(), 1,val,8);
	        	// ControllerPlugin.showToast('showMessage', '8');
	      }
	      
			$('#'+this.element.attr('id')+'-perc').html(val);				
